...
public class ExpansionSingleExpansionRM1Builder implements ExpansionRM1Builder {
	...
	private DocScorer queryScorerDoc;
	private DocScorer expansionScorer;
	private DocScorer queryScorerExpansion;
	...
	public ExpansionSingleExpansionRM1Builder(GQuery query,
			SearchHits initialHits, 
			DocScorer docScorer, 
			DocScorer queryScorerDoc,
			DocScorer expansionScorer,
			DocScorer queryScorerExpansion,
			TermCollector termCollector,
			int feedbackDocs,
			int feedbackTerms) {

		this.docScorer = docScorer;
		this.queryScorerDoc = queryScorerDoc;
		this.expansionScorer = expansionScorer;
		this.queryScorerExpansion = queryScorerExpansion;
	...
	public ExpansionSingleExpansionRM1Builder(GQuery query,
			...
			DocScorer queryScorerExpansion,
			TermCollector termCollector) {
		this(query, initialHits, docScorer, queryScorerDoc,
				expansionScorer, queryScorerExpansion, termCollector,
	...
	public FeatureVector buildRelevanceModel(Stopper stopper, Map<String, Double> params) {
		...
		while (hitIt.hasNext() && i < feedbackDocs) {
			...
			queryScorers.put(queryScorerDoc, params.get(EntityRunner.DOCUMENT_WEIGHT));
			queryScorers.put(queryScorerExpansion, params.get(EntityRunner.EXPANSION_WEIGHT));
